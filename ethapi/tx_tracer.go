...
func (s *PublicTxTraceAPI) traceTx(
	...
	state.Finalise(true)
	if err != nil {
		errTrace := txtracer.GetErrorTraceFromMsg(&msg, block.Hash, *block.Number, tx.Hash(), index, err)
		at := make([]txtracer.ActionTrace, 0)
	...
	if result != nil && result.Err != nil {
		if len(*traceActions) == 0 {
			...
			errTrace := txtracer.GetErrorTraceFromMsg(&msg, block.Hash, *block.Number, tx.Hash(), index, result.Err)
			at := make([]txtracer.ActionTrace, 0)
			at = append(at, *errTrace)
			return &at, nil
		}
...
func (s *PublicTxTraceAPI) traceBlock(ctx context.Context, block *evmcore.EvmBlock, txHash *common.Hash, traceIndex *[]hexutil.Uint) (*[]txtracer.ActionTrace, error) {
	var (
		blockNumber   int64
		...
		for i, tx := range block.Transactions {
			if txHash == nil || *txHash == tx.Hash() {
				...
				if tx.To() != nil && *tx.To() == sfc.ContractAddress {
					errTrace := txtracer.GetErrorTrace(block.Hash, *block.Number, &from, tx.To(), tx.Hash(), index, errors.New("sfc tx"))
					at := make([]txtracer.ActionTrace, 0)
					at = append(at, *errTrace)
	...
	if len(callTrace.Actions) == 0 {
		if traceIndex != nil || txHash != nil {
			...
			emptyTrace := txtracer.CallTrace{
				Actions: make([]txtracer.ActionTrace, 0),
			}
			blockTrace := txtracer.NewActionTrace(block.Hash, *block.Number, common.Hash{}, 0, "empty")
			txAction := txtracer.NewAddressAction(&common.Address{}, 0, []byte{}, nil, hexutil.Big{}, nil)
 ...
 */
func (s *PublicTxTraceAPI) Block(ctx context.Context, numberOrHash rpc.BlockNumberOrHash) (*[]txtracer.ActionTrace, error) {
...
func (s *PublicTxTraceAPI) Transaction(ctx context.Context, hash common.Hash) (*[]txtracer.ActionTrace, error) {
	defer func(start time.Time) {
...
func (s *PublicTxTraceAPI) Get(ctx context.Context, hash common.Hash, traceIndex []hexutil.Uint) (*[]txtracer.ActionTrace, error) {
	defer func(start time.Time) {
...
func (s *PublicTxTraceAPI) traceTxHash(ctx context.Context, hash common.Hash, traceIndex *[]hexutil.Uint) (*[]txtracer.ActionTrace, error) {
...
func (s *PublicTxTraceAPI) Filter(ctx context.Context, args FilterArgs) (*[]txtracer.ActionTrace, error) {
	...
	if args.Count == 0 {
		...
		results := make(chan txtracer.ActionTrace, 100000)
...
func worker(id int,
	...
	results chan<- txtracer.ActionTrace,
